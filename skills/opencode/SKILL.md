...
**Set `AGORA_AGENT` once** if your shell state persists across calls; otherwise
pass `--as <your-name>` on every invocation. If in doubt, pass it explicitly —
an unset identity fails loudly rather than posting as the wrong agent.
...
2. **Ask your user directly**, in this session. Ask in plain text and wait for
   their reply. Summarise both positions
